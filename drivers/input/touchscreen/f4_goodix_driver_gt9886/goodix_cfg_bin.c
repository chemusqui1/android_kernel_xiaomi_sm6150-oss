...
int goodix_start_cfg_bin(struct goodix_ts_core *ts_core)
{
	...
	ts_err("enter::%s\n", __func__);
	cfg_bin_thrd =
	    kthread_run(goodix_cfg_bin_proc, ts_core, "goodix-parse_cfg_bin");
...
int goodix_get_lockdowninfo(struct goodix_ts_core *ts_core)
{
	...
	ts_info
	    ("lockdown is:0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",
...
int goodix_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin)
{
	...
	if (cfg_bin->bin_data_len != cfg_bin->head.bin_len) {
		ts_err
		    ("cfg_bin length error,len in cfg_bin:%d, len of firmware:%d",
	...
	if (checksum != cfg_bin->head.checksum) {
		ts_err
		    ("cfg_bin checksum ERROR, checksum in cfg_bin:0x%02x, checksum caculate:0x%02x",
	...
	cfg_bin->cfg_pkgs =
	    kzalloc(sizeof(struct goodix_cfg_package) * cfg_bin->head.pkg_num,
		    GFP_KERNEL);
	if (!cfg_bin->cfg_pkgs) {
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		if (i == cfg_bin->head.pkg_num - 1) {
			...
			if (offset2 <= offset1) {
				ts_err
				    ("offset error,pkg:%d, offset1:%d, offset2:%d",
				     i, offset1, offset2);
				r = -EINVAL;
				goto exit;
			}
...
int goodix_cfg_bin_proc(void *data)
{
	...
	if (!r) {
		...
		if (r != -EBUS) {
			ts_err
			    ("get reg and cfg from cfg_bin FAILED, update fw then retry");
			...
			ts_err
			    ("get reg and cfg from cfg_bin FAILED, I2C com ERROR");
			goto exit;
		}
	}
	/*init i2c_set_doze_mode para */
			   ...
			   struct goodix_cfg_bin *cfg_bin)
{
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		if (!strncmp(cfg_bin->cfg_pkgs[i].cnst_info.ic_type, "nanjing",
			...
			ts_dev->ic_type = IC_TYPE_NANJING;
			 ...
			 (cfg_bin->cfg_pkgs[i].cnst_info.ic_type, "normandy",
			  sizeof(cfg_bin->cfg_pkgs[i].cnst_info.ic_type)))
			ts_dev->ic_type = IC_TYPE_NORMANDY;
		else
			ts_err
			    ("get ic type FAILED, unknow ic type from cfg_bin:%s",
			     cfg_bin->cfg_pkgs[i].cnst_info.ic_type);
		...
		if (r < 0) {
			ts_err
			    ("read sensor id FAILED,I2C ERROR, pkg:%d, sensor_id reg:0x%02x",
			     i, addr);
			goto exit;
		} else {
			...
			if (cfg_bin->cfg_pkgs[i].reg_info.sensor_id.reserved1 !=
			    0)
				temp_sensor_id &=
				    cfg_bin->cfg_pkgs[i].reg_info.sensor_id.
				    reserved1;
			if (temp_sensor_id !=
			    cfg_bin->cfg_pkgs[i].cnst_info.sensor_id) {
				ts_err
				    ("pkg:%d, sensor id contrast FAILED, reg:0x%02x",
				     i, addr);
				ts_err
				    ("sensor_id from i2c:%d, sensor_id of cfg bin:%d",
				     temp_sensor_id,
				     cfg_bin->cfg_pkgs[i].cnst_info.sensor_id);
				continue;
			}
		...
		if (!addr || !cfg_bin->cfg_pkgs[i].cnst_info.fw_mask[0]) {
			...
			if (r < 0) {
				ts_err
				    ("read fw_mask FAILED, I2C ERROR, pkg: %d, fw_mask reg:0x%02x",
				     i, addr);
				goto exit;
			} else
			    if (strncmp
				(temp_fw_mask,
				 ...
				 sizeof(temp_fw_mask))) {
				ts_err
				    ("pkg:%d, fw_mask contrast FAILED, reg:0x%02x,",
				     i, addr);
		...
		if (r < 0) {
			ts_err
			    ("read pid FAILED, I2C ERROR, pkg: %d, pid reg:0x%02x",
			     i, addr);
			goto exit;
		} else
		    if (strncmp
	...
	if (high_sense_pkg) {
		...
		if (!ts_dev->highsense_cfg) {
			...
			if (!ts_dev->highsense_cfg) {
				ts_err
				    ("Failed to alloc memory for high sense cfg");
				return -ENOMEM;
			}
...
int goodix_read_cfg_bin(struct device *dev, struct goodix_cfg_bin *cfg_bin)
{
	...
	for (i = 0; i < TS_RQST_FW_RETRY_TIMES; i++) {
		...
		if (r < 0) {
			ts_err
			    ("Cfg_bin image [%s] not available,error:%d, try_times:%d",
			     cfg_bin_name, r, i + 1);
			msleep(1000);
		} else {
